...
const Bio = () => {
  return (
    <>
      <div className='bg-gray-300 w-full lg:h-500 h-700'>
        {/* Intro */}
        <div className='flex flex-col w-full justify-between items-center h-full '>
          {/* Title */}
            <div className='w-full h-1/4 lg:h-2/4 flex flex-col justify-end items-start'>
              <div className='w-1/2 ml-4 h-auto  flex flex-col '>
                <h2 className='text-rosered text-3xl lg:text-8xl font-clash'>Royce Williams</h2>
                <p className='font-azeret text-xs lg:text-sm text-black'>Computer Engineer & Founder of REVITAL</p>
              </div>
            </div>
          {/* Bio */}
      ...
      <div className='h-screen w-full bg-gray-300 flex'>
          <div className='w-1/2 h-full flex justify-center mt-4'>
            <div className='w-full ml-4 h-auto'>
              <h2 className='font-clash text-5xl lg:text-9xl text-rosered'>Here to Support You With</h2>
            </div>
          </div>
          <div className='w-full lg:w-1/2  h-full flex justify-center items-center'>
            <div className='w-full h-1/2 lg:h-2/3 flex flex-col justify-evenly items-center'>
              <div className='w-full lg:w-3/4 lg:h-28 text-black font-azeret'> 
                <h1 className=' w-full text-lg lg:text-2xl '>
                  <div className='w-4 h-4 text-sm flex items-center justify-center border-black border rounded-full'>1</div> Design</h1>
                  <p className='text-xxs lg:text-xs'>With a fresh approach to web and app design, I bring creative and user-centric digital solutions to the table. Crafting a compelling brand identity is the cornerstone of a successful online presence, and I'm here to help you build that foundation.</p>
              </div>
              <div className='w-full lg:w-3/4 lg:h-28 text-black font-azeret'>
                <h1 className='w-full text-lg lg:text-2xl '> <div className='w-4 h-4 text-sm flex items-center justify-center border-black border rounded-full'>2</div>  Development</h1>
                <p className='text-xxs lg:text-xs'>I specialize in developing scalable websites & software from the ground up, ensuring they seamlessly integrate with your companies vision.</p>
              </div>
              <div className='w-full lg:w-3/4 lg:h-28 text-black font-azeret'>
                <h1 className='w-full text-lg lg:text-2xl '>
                <div className='w-4 h-4 text-sm flex items-center justify-center border-black border rounded-full'>3</div> 
                  Full Package</h1>
                  <p className='text-xxs lg:text-xs'>What sets me apart is my ability to take your idea from concept to reality, providing a comprehensive solution from start to finish.</p>
              </div>
            </div>
          </div>
      </div>  
      ...
      <div className=' w-full bg-gray-300 flex flex-col min-h-screen items-center justify-center font-bold text-white'>
              <div className="text-center text-4xl lg:text-8xl text-rosered font-clash font-regular">
              <Navbar /> Building..
